...
export const HandleDestination = (req, file, cb) => {
    ...
    // 检查是否存在该目录
    const isdir = fs.existsSync(join(__dirname, `${isenv == 'dev' ? dev_path : pro_path}www/public/${file.fieldname}/${date}`));
    let dir_path = join(__dirname, `${isenv == 'dev' ? dev_path : pro_path}www/public/${file.fieldname}/${date}`);
    if (!isdir) {
        // 创建目录
